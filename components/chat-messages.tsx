  ...
  return (
    <div className="relative mx-auto px-4 w-full">
      {messages.map(message => (
        // --- НАЧАЛО ИЗМЕНЕНИЯ ---
        // Добавляем класс анимации `animate-fadeInUp` к контейнеру каждого сообщения.
        <div 
          key={message.id} 
          className={cn(
            "mb-4 flex flex-col gap-4",
            // Применяем анимацию только к сообщениям от ассистента (ИИ)
            message.role === 'assistant' && 'animate-fadeInUp'
          )}
          // Добавляем inline-стили для задержки, чтобы анимация была плавной
